...
class ExcelProcessor
{
    ...
    private $spreadsheet;
    private $aiService;
    private $maxExecutionTime = 900; // 15 minutes for better timeout handling
    private $maxMemoryLimit = '4096M'; // Increased memory limit
    private $chunkSize = 3; // Smaller chunks for better memory management
    ...
    {
        Log::info('=== EXCEL PROCESSOR START ===');
        ...
        try {
            ...
            if (!$outputFileName) {
                ...
                Log::info('Generated output filename: ' . $outputFileName);
            }
            ...
            $this->clearAllPhotos();
            Log::info("Photos cleared successfully");
            
            // Save copy template tanpa foto with optimized settings
            Log::info("Creating Excel writer");
            $writer = IOFactory::createWriter($this->spreadsheet, 'Xlsx');
            Log::info("Setting writer options");
            $writer->setPreCalculateFormulas(false);
            $writer->setUseDiskCaching(true);
            Log::info("Saving file to: " . $this->outputPath);
            ...
            Log::info('=== EXCEL PROCESSOR SUCCESS ===');
            Log::info('Result: ' . json_encode($result));
            
            return $result;
            
        } catch (\Exception $e) {
            Log::error('=== EXCEL PROCESSOR ERROR ===');
            ...
            Log::error('Error result: ' . json_encode($errorResult));
            return $errorResult;
        }
    }
    ...
    public function processPhotoUrls($photoUrls, $templateAnalysis, $progressCallback = null)
    {
        ...
        foreach ($photoUrls as $url) {
            ...
            try {
                ...
                $placementResult = $this->insertPhotoToCell($photoData['path'], $placement['worksheet'], $placement['cell']);
     ...
     */
    private function analyzeImageWithGemini($imageData)
    {
        try {
            // Save image data to temporary file
            $tempPath = tempnam(sys_get_temp_dir(), 'img_analysis_') . '.jpg';
            file_put_contents($tempPath, $imageData);
            
            // Create analysis prompt
            $prompt = "Analyze this tower survey photo and categorize it. Identify if this is a tower structure, equipment, environment, site condition, or general photo. Provide a brief description in Indonesian.";
            
            // Use AIService to analyze the image
            $analysisResult = $this->aiService->analyzeImageWithPrompt($tempPath, $prompt);
            
            // Clean up temporary file
            unlink($tempPath);
            
            // Extract category from analysis
            $category = $this->extractCategoryFromAnalysis($analysisResult);
            ...
            Log::error('Gemini image analysis failed: ' . $e->getMessage());
            return [
                'success' => false,
                'error' => 'Gagal menganalisis gambar: ' . $e->getMessage(),
                'category' => 'general'
            ];
        }
    }
    ...
    /**
     * Find best placement for photo based on category
     */
    private function findBestPlacement($category)
    {
        // Default placement mapping for ESR template
        $defaultPlacements = [
            'tower_structure' => [
                'worksheet' => 'ESR',
                'cell' => 'B10',
        ...
        return $defaultPlacements[$category] ?? $defaultPlacements['general'];
    }
    ...
    /**
     * Process local photos from uploaded files
     */
    public function processLocalPhotos($filePaths, $templatePath = null)
    {
        try {
            $startTime = microtime(true);
            $this->logSystemResources('Start Local Processing');
            
            // Load template
            if (!$templatePath) {
                $templatePath = storage_path('app/templates/template_survey.xlsx');
            }
            ...
            foreach ($filePaths as $index => $filePath) {
                if (!file_exists($filePath)) {
                    $invalidFiles[] = ['index' => $index, 'path' => $filePath, 'reason' => 'File not found'];
                    continue;
                }
                ...
                if (!$imageInfo) {
                    $invalidFiles[] = ['index' => $index, 'path' => $filePath, 'reason' => 'Not a valid image'];
                    continue;
                }
                ...
                if ($fileSize > 10 * 1024 * 1024) {
                    $invalidFiles[] = ['index' => $index, 'path' => $filePath, 'reason' => 'File too large (max 10MB)'];
                    continue;
                }
            ...
            Log::info("Processing " . count($validFiles) . " local files");
            
            // Process files in batches
            $batchSize = 5; // Smaller batch for local files
            $processedPhotos = [];
            $totalProcessed = 0;
            $batchesProcessed = 0;
            ...
            foreach ($batches as $batchIndex => $batch) {
                ...
                foreach ($batch as $filePath) {
                    try {
                        ...
                        // Analyze image
                        $analysis = $this->analyzeImageWithGemini($imageData);
                        
                        if ($analysis['success']) {
                            // Find placement
                            $placement = $this->findBestPlacement($analysis['category']);
                            
                            // Place in Excel
                            $placementResult = $this->placePhotoInExcel($imageData, $placement);
                            ...
                            $batchResults[] = [
                                ...
                                'analysis' => $analysis,
                                'placement' => $placement,
                                'placement_success' => $placementResult['success'],
                                'success' => true
                            ];
                        ...
                        Log::error("Error processing local file {$filePath}: " . $e->getMessage());
                        $batchResults[] = [
                            'file_path' => $filePath,
                            'filename' => basename($filePath),
                            'error' => $e->getMessage(),
                ...
                $processedPhotos = array_merge($processedPhotos, $batchResults);
                $batchesProcessed++;
                
                $this->logSystemResources("After batch {$batchesProcessed}");
            ...
            Log::info("Local photo processing completed", [
            ...
            Log::error('Error in local photo processing: ' . $e->getMessage());
            return [
                'success' => false,
                'error' => $e->getMessage()
            ];
